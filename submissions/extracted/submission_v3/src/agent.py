    ...
    for attempt in (1, 2):
        ...
        try:
            ...
            if attempt == 1:
                ...
                try:
                    print("[Agent] Writing fallback baseline...", flush=True)
                    _write_fallback(workspace_task_dir, output_path, task_config)
                    print("[Agent] Fallback written", flush=True)
                    return 0
                except Exception as fb_exc:
                    (output_dir / "baseline_error.txt").write_text(msg + f"\nFallback: {fb_exc}\n", encoding="utf-8")
                    return 1
    return 1

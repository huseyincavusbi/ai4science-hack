...
- Step 1: Check covariate balance. Compute standardized mean diffs treated vs control.
- Step 2: Fit propensity score model (GradientBoostingClassifier predicting treatment from X).
- Step 3: Build MULTIPLE estimators and ensemble:
  - T-learner: GradientBoostingRegressor per group (300 trees, max_depth=4, lr=0.05)
  - X-learner: T-learner + propensity weighting, better with imbalanced groups
  - Doubly Robust: outcome model + propensity model, robust to misspecification
  - Causal Forest if available (sklearn doesn't have it but causalml does)
- Step 4: Ensemble predictions (average of T/X/DR) for stability.
- Step 5: Evaluate on held-out train split. Report ATE and per-unit variance.
- IMPORTANT: Use random_state=42 for reproducibility. Cross-validate hyperparams.

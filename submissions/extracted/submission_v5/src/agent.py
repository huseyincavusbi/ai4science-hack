...
- Step 1: Check covariate balance (standardized mean diff treated vs control).
- Step 2: Fit propensity score model (LogisticRegression predicting treatment from covariates).
- Step 3: Try estimators:
  - T-learner: model_treated(X), model_control(X), effect = diff
  - X-learner: T-learner + propensity weighting
  - Doubly Robust: combines outcome + propensity models
- Step 4: Evaluate on held-out train split. Compare ATE.

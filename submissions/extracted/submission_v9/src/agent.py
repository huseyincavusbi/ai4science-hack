...
- Step 2: Fit propensity score (GradientBoostingClassifier, 200 trees, max_depth=3).
  Clip to [0.05, 0.95] to avoid extreme weights.
- Step 3: T-learner — GradientBoostingRegressor(300 trees, max_depth=4, lr=0.05)
  separately on T=1 and T=0. Effect = mu1 - mu0.
- Step 4: Optionally add X-learner (propensity-weighted) and Doubly Robust with
  cross-fitting (5-fold). If ensembling: weight DR=0.5, T=0.25, X=0.25
  (DR is most robust to misspecification).
- Step 5: Evaluate on held-out train split (20%). Report ATE and per-unit stats.
- Step 6: Write predictions.csv with id,prediction (float per-unit effect). NOT probability.
...
## Code Requirements
- ALWAYS use os.walk to find data files, never hardcode "data/train.csv".
  Start every script with a find_file() helper function. Example:
  def find_file(name):
...
- The working directory on Codabench is NOT the task directory. Use find_file().
